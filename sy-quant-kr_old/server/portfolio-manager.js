...
/*
 * 신규매수 예상손실이 MASTER 일일 손실한도의 남은 여유보다 크면
 * 주문 직전에 차단한다.
 *
 * OPEN은 실제 손절 -1.0%를 유지하되 장초 시세 갱신 사이의 미끄러짐을
 * 감안해 위험예산 계산에서만 0.30%p를 더한 1.30%를 사용한다.
 *
 * 다른 전략은 기존 동작을 바꾸지 않는다.
 * 향후 필요하면 canBuy(..., { dailyRiskGuard })로 전략별 위험률을 전달할 수 있다.
 */
const DEFAULT_DAILY_RISK_GUARDS = Object.freeze({
  OPEN: Object.freeze({
    enabled: true,
    dailyLossLimitRate: 0.01,
    estimatedLossRatePercent: 1.30
  })
});
...
function getTodayRealizedProfit(state, date = todayKey()) {
  ensureMasterState(state);
  ...
  return state.tradeLogs
    .filter(log => {
      ...
      return (
        ...
        log.profit !== null &&
        Number.isFinite(toNumber(log.profit))
      );
    })
    .reduce(
 ...
 * MASTER 일일 손실예산 스냅샷.
 *
 * riskBasisProfit은 오늘 실현손익과 오늘 총자산손익 중 더 나쁜 값을 사용한다.
 * - 실현손실이 큰데 다른 보유종목 평가이익이 있다고 해서 신규 위험예산을 늘리지 않는다.
 * - 반대로 보유 평가손실이 더 크면 그 손실까지 반영한다.
 *
 * 또한 당일 이익이 났더라도 기본 일일손실한도 이상으로 위험예산을 늘리지 않는다.
 */
function getDailyLossRiskSnapshot(state, options = {}) {
  ensureMasterState(state);

  const date = String(options.date || todayKey()).slice(0, 10);
  const stateDailyStartDate = String(state.dailyStartDate || "").slice(0, 10);

  const optionDailyStartAsset = Math.max(
    0,
    toNumber(options.dailyStartAsset)
  );
  const savedDailyStartAsset = Math.max(
    0,
    toNumber(state.dailyStartAsset)
  );

  const dailyStartAsset =
    optionDailyStartAsset > 0
      ? optionDailyStartAsset
      : savedDailyStartAsset;

  const hasValidDailyStart =
    dailyStartAsset > 0 &&
    (!stateDailyStartDate || stateDailyStartDate === date);

  const baseAsset = hasValidDailyStart
    ? dailyStartAsset
    : Math.max(
        1,
        toNumber(options.baseAsset),
        toNumber(state.initialCapital, MASTER_INITIAL_CAPITAL)
      );

  const savedLimit = Math.max(
    0,
    toNumber(state.dailyLossLimit)
  );
  const optionLimit = Math.max(
    0,
    toNumber(options.dailyLossLimit)
  );
  const dailyLossLimitRate = Math.max(
    0,
    toNumber(options.dailyLossLimitRate)
  );
  ...
  const dailyLossLimit =
    optionLimit > 0
      ...
      : savedLimit > 0
        ? savedLimit
        : baseAsset * dailyLossLimitRate;

  const todayRealizedProfit =
    getTodayRealizedProfit(state, date);

  const currentAsset = getEquity(state);
  const todayAssetProfit = hasValidDailyStart
    ? currentAsset - dailyStartAsset
    : todayRealizedProfit;

  const riskBasisProfit = Math.min(
    todayRealizedProfit,
    todayAssetProfit
  );
  ...
  const remainingLossBudget = Math.max(
    0,
    dailyLossLimit + Math.min(0, riskBasisProfit)
  );

  const requestedAmount = Math.max(
    0,
    toNumber(options.requestedAmount)
  );
  const estimatedLossRatePercent = Math.max(
    0,
    toNumber(options.estimatedLossRatePercent)
  );
  const estimatedLossAmount =
    requestedAmount * estimatedLossRatePercent / 100;

  const enabled =
    options.enabled !== false &&
    dailyLossLimit > 0 &&
    estimatedLossRatePercent > 0;

  const blocked =
    enabled &&
    (
      remainingLossBudget <= 0 ||
      estimatedLossAmount >= remainingLossBudget
    );

  const reason = blocked
    ? (
        `MASTER 잔여 손실예산 부족 / ` +
        `기준손익 ${Math.round(riskBasisProfit).toLocaleString()}원 / ` +
        `실현손익 ${Math.round(todayRealizedProfit).toLocaleString()}원 / ` +
        `잔여 ${Math.round(remainingLossBudget).toLocaleString()}원 / ` +
        `신규 예상위험 ${Math.round(estimatedLossAmount).toLocaleString()}원 ` +
        `(${estimatedLossRatePercent.toFixed(2)}%) / ` +
        `일일한도 ${Math.round(dailyLossLimit).toLocaleString()}원`
      )
    : "정상";
  ...
  return {
    ...
    blocked,
    enabled,
    code: blocked ? "DAILY_LOSS_RISK_BUDGET" : null,
    reason,
    date,
    dailyStartAsset: hasValidDailyStart ? dailyStartAsset : null,
    currentAsset,
    todayAssetProfit,
    todayRealizedProfit,
    riskBasisProfit,
...
function canBuy(state, options = {}) {
  ...
  /*
   * 서버의 /api/core-paper-buy가 portfolioManager.canBuy()를 호출하는 순간
   * 최신 MASTER 상태 기준으로 다시 계산한다.
   * OPEN은 별도 전달값이 없어도 1.30% 기본 위험률을 적용하므로
   * 다른 전략과 동시 주문이 있어도 최종 주문 직전에 차단된다.
   */
  const suppliedDailyRiskGuard =
    options.dailyRiskGuard &&
    typeof options.dailyRiskGuard === "object"
      ? options.dailyRiskGuard
      : null;

  const defaultDailyRiskGuard =
    DEFAULT_DAILY_RISK_GUARDS[strategy] || null;

  const dailyRiskGuard =
    suppliedDailyRiskGuard ||
    defaultDailyRiskGuard;

  let dailyRisk = null;

  if (
    dailyRiskGuard &&
    dailyRiskGuard.enabled !== false
  ) {
    dailyRisk = getDailyLossRiskSnapshot(
      state,
      {
        ...dailyRiskGuard,
...
function requestSell(state, options = {}) {
  ...
  const fullQty = getHoldingQty(holding);
  if (fullQty <= 0) {
    return {
      ok: false,
      reason: "보유수량 오류"
  ...
  // qty 미지정은 기존과 동일하게 전량매도.
  // WAVE 부분익절처럼 qty를 지정한 경우에만 선택수량을 매도한다.
  const requestedSellQty = Math.floor(toNumber(options.qty));
  const qty = requestedSellQty > 0
    ? Math.min(fullQty, requestedSellQty)
    : fullQty;
  ...
  const remainingQty = Math.max(0, fullQty - qty);
  if (remainingQty <= 0) {
    state.holdings = state.holdings.filter(item => item !== holding);
  } else {
    holding.qty = remainingQty;
    holding.buyAmount = buyPrice * remainingQty;
    holding.partialSold = true;
    holding.partialSoldAt = options.time || nowText();
    holding.partialSoldAtMs = timestampMs;
  }
...
module.exports = {
  ...
  getTodayRealizedProfit,
  getDailyLossRiskSnapshot,
  getTradingSettings,
