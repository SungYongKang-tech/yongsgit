...
function loadState() {
  ...
  for (const strategyGroup of ["CORE", "VOLUME"]) {
    ...
    if (
      !state.buyDecisionStats[strategyGroup].rejected ||
      typeof state.buyDecisionStats[strategyGroup].rejected !== "object"
    ) {
      state.buyDecisionStats[strategyGroup].rejected = {};
    }
  }
...
function recordBuyDecision(
  ...
  if (!["CORE", "VOLUME"].includes(strategyGroup)) {
    return;
  }
  ...
  const stats = state.buyDecisionStats[strategyGroup];
  ...
  stats.rejected[category] =
    Number(stats.rejected[category] || 0) + 1;

  // 전체검색과 후보재평가 출처도 함께 집계
  const sourceKey =
    source === "WATCH"
      ? "후보재평가"
      : "전체검색";
...
function logBuyDecisionSummary(state) {
  ...
  for (const strategyGroup of [
    ...
    console.log(
      `[${strategyGroup} 판단통계] ` +
      `검사 ${Number(stats.checked || 0)}건 / ` +
      `통과 ${Number(stats.passed || 0)}건 / ` +
      `매수 ${Number(stats.bought || 0)}건 / ` +
      `${rejectText}`
    );
  }
}
...
function judgeOpenBuy(state, item, price) {
 ...
 if (discoverScore < settings.openMinDiscoverScore) {
  return {
    ...
    reason:
      `발견점수 ${discoverScore.toFixed(1)} / ` +
      `OPEN 최소기준 ${settings.openMinDiscoverScore.toFixed(1)} / ` +
      `부족 ${(settings.openMinDiscoverScore - discoverScore).toFixed(1)}점`
  };
}
  if (
  changeRate < settings.openMinChangeRate ||
  changeRate > settings.openMaxChangeRate
) {
  return {
    pass: false,
  ...
  return {
    ...
    reason: makeVolumeRatioLog(
      ...
      "OPEN",
      volumeRatio,
      settings.openMinTradeVolumeRatio
    )
  };
}
  if (
  dayPosition < settings.openMinDayPositionRate ||
  dayPosition > settings.openMaxDayPositionRate
) {
  return {
    pass: false,
    reason: makeMinMaxLog(
  ...
  if (
  openPosition < settings.openMinOpenPositionRate ||
  openPosition > settings.openMaxOpenPositionRate
) {
  return {
    pass: false,
    reason: makeMinMaxLog(
...
async function runOpenBuyOnce() {
 ...
 const candidates = await discoverCandidates(state, "OPEN");
  const passed = [];
  let logged = 0;
...
function isVolumeCandidateGettingStronger(
  ...
  // 1분이 지난 경우에만 다음 비교 기준으로 갱신
  state.volumeCandidateHistory[code] =
    current;

  const scoreDiff =
    current.score - Number(prev.score || 0);
  ...
  const priceDiffRate =
    Number(prev.price || 0) > 0
      ? (
          (current.price - Number(prev.price)) /
          Number(prev.price)
        ) * 100
      : 0;
  ...
  if (volumeDiff < -50) {
    return {
      ...
      reason:
        `거래량 급감 ` +
        `${Number(prev.volumeRatio || 0).toFixed(1)}% → ` +
...
updateCandidateWatchList(
  ...
  strategyGroup
    ...
    console.log(
      ...
      `${price.toLocaleString()}원(${priceDiffRate.toFixed(2)}%) / ` +
      `점수 ${Number(candidate.firstDiscoverScore || 0).toFixed(1)}→` +
...
async function runBuyOnce() {
  ...
  if (isOpenBlockingCoreVolume(state)) {
    console.log(
      "[BUY] OPEN 진행 또는 대기 중 / CORE·VOLUME 신규매수 보류"
    );
    saveState(state);
    return;
  }
