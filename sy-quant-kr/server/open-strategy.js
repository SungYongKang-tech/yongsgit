...
// 2026-09-04 OPEN 관찰고점 보호
// 고점대비 -2.5% 이하: 신규매수 차단
// -1.5~-2.5%:
// 최근 저점대비 +0.8% 이상 회복
// + 직전확인대비 +0.10% 이상 상승 필요
openObservationPeakCautionDrawdownRate: -1.50,
openObservationPeakHardBlockRate: -2.50,
openObservationPeakRecoveryMinRate: 0.80,
openObservationPeakRecentRiseMinRate: 0.10,


  openMinHoldingSeconds: 120,
openStopLossRate: -1.0,
// 실제 손절값은 그대로 두고 신규매수 위험예산 계산에만 0.30%p 여유를 더한다.
openStopLossRiskBufferRate: 0.30,
...
// OPEN 단계형 시장모드
// 0~19점: 절대차단
// 20~24점: 초약세장 초강력 후보만, 하루 최대 2종목
// 25~39점: 기존 약세장 초강력 후보만, 하루 최대 3종목
// 40점 이상: 기존 정상 OPEN, 런타임 설정 한도(최대 5종목)
openMarketHardBlockScore: 40,
openMarketAbsoluteBlockScore: 20,

openMarketTierUltraWeakMaxScore: 24,
openMarketTierUltraWeakMaxBuyCount: 2,
openMarketTierWeakMaxScore: 39,
openMarketTierWeakMaxBuyCount: 3,
openMarketTierNormalMaxBuyCount: 5,

// 20~24점 초약세장에서는 기존 초강력 조건 + 매수직전 흐름 재확인
openUltraWeakMinRecentPriceDiffRate: 0.0,
openUltraWeakMinDayPositionRate: 65,
openUltraWeakMaxDayPositionDrop: 20,
openUltraWeakRecentSampleCount: 6,

// 20~39점 약세장에서도 종목 자체 흐름이 압도적으로 강한 경우만 예외 허용
...
function getOpenExpectedLossRatePercent() {
  return (
    Math.abs(Number(settings.openStopLossRate || 0)) +
    Math.max(0, Number(settings.openStopLossRiskBufferRate || 0))
  );
}

function getOpenDailyRiskGuard(state = {}) {
  return {
    enabled: true,
    dailyLossLimit: Number(state.dailyLossLimit || 0),
    dailyLossLimitRate: Number(settings.dailyLossLimitRate || 0.01),
    dailyStartAsset: Number(state.dailyStartAsset || 0),
    baseAsset: Number(
    ...
    estimatedLossRatePercent:
      getOpenExpectedLossRatePercent()
  };
}

function checkOpenPreBuyRiskBudget(state, buyAmount) {
  return portfolioManager.getDailyLossRiskSnapshot(
    state,
    {
      ...getOpenDailyRiskGuard(state),
      requestedAmount: Number(buyAmount || 0)
    }
  );
}
...
function getOpenMarketTierInfo() {
  const marketData = loadOpenMarketData();
  const available = marketData?.available === true;
  const marketScore = available ? Number(marketData.marketScore || 0) : null;
  ...
  if (!available) {
    return {
      ...
      marketScore: null,
      tier: "NO_DATA",
      tierMaxBuyCount: Number(settings.openMarketTierNormalMaxBuyCount || 5)
    };
  }

  if (marketScore < Number(settings.openMarketAbsoluteBlockScore || 20)) {
    return { available: true, marketScore, tier: "ABSOLUTE_BLOCK", tierMaxBuyCount: 0 };
  }

  if (marketScore <= Number(settings.openMarketTierUltraWeakMaxScore || 24)) {
    return {
      available: true,
      marketScore,
      tier: "ULTRA_WEAK",
      tierMaxBuyCount: Number(settings.openMarketTierUltraWeakMaxBuyCount || 2)
    };
  }

  if (marketScore <= Number(settings.openMarketTierWeakMaxScore || 39)) {
    return {
      available: true,
      marketScore,
      tier: "WEAK",
      tierMaxBuyCount: Number(settings.openMarketTierWeakMaxBuyCount || 3)
    };
  }
...
function isOpenBuyCapacityFull(state) {
  const runtime = getOpenRuntimeSettings(state);
  const effectiveMax = getOpenEffectiveMaxBuyCount(state);
  const holdingFull = getOpenHoldingCount(state) >= Number(runtime.maxHoldingCount || 0);
  const dailyFull = getOpenBuyCountToday(state) >= effectiveMax;
  return holdingFull || dailyFull || effectiveMax <= 0;
}
...
function evaluateOpenWeakMarketStrongOverride(state, item, marketData) {
  ...
  const absoluteBlockScore = Number(settings.openMarketAbsoluteBlockScore || 20);
  ...
  const baseStrongPass =
    settings.openWeakMarketStrongOverrideEnabled === true &&
    inOverrideZone &&
    discoverScore >= Number(settings.openWeakMarketStrongMinDiscoverScore || 9) &&
    Number(momentum.momentumScore || 0) >= Number(settings.openWeakMarketStrongMinMomentumScore || 75) &&
    Number(momentum.pricePersistence || 0) >= Number(settings.openWeakMarketStrongMinPricePersistence || 0.75) &&
    Number(momentum.volumePersistence || 0) >= Number(settings.openWeakMarketStrongMinVolumePersistence || 0.75) &&
    observationCount >= Number(settings.openWeakMarketStrongMinObservationCount || 4) &&
    Number(momentum.strongCount || 0) >= Number(settings.openWeakMarketStrongMinStrongObservationCount || 3);

  let ultraWeakPass = true;
  let recentPriceDiffRate = 0;
  let currentDayPosition = 0;
  let peakDayPosition = 0;
  let dayPositionDrop = 0;

  const ultraWeak =
    marketData?.available === true &&
    marketScore >= absoluteBlockScore &&
    marketScore <= Number(settings.openMarketTierUltraWeakMaxScore || 24);

  if (ultraWeak) {
    const count = Math.max(2, Number(settings.openUltraWeakRecentSampleCount || 6));
    ...
    const currentPrice = Number(current?.price || item?.currentPrice || item?.price || 0);
    const previousPrice = Number(previous?.price || 0);

    recentPriceDiffRate = previousPrice > 0
      ? ((currentPrice - previousPrice) / previousPrice) * 100
      : 0;

    currentDayPosition = Number(
      current?.dayPosition ?? item?.dayPositionRate ?? item?.dayPosition ?? 0
    );

    peakDayPosition = recent.length
      ? Math.max(...recent.map(row => Number(row?.dayPosition || 0)))
      : currentDayPosition;

    dayPositionDrop = Math.max(0, peakDayPosition - currentDayPosition);

    ultraWeakPass =
      recentPriceDiffRate >= Number(settings.openUltraWeakMinRecentPriceDiffRate || 0) &&
      currentDayPosition >= Number(settings.openUltraWeakMinDayPositionRate || 65) &&
      dayPositionDrop <= Number(settings.openUltraWeakMaxDayPositionDrop || 20);
  }

  const pass = baseStrongPass && ultraWeakPass;
  ...
  return {
    ...
    reason:
      ...
      (ultraWeak
        ? ` / 초약세재확인 ${ultraWeakPass ? "통과" : "실패"} / ` +
          `직전가격 ${recentPriceDiffRate >= 0 ? "+" : ""}${recentPriceDiffRate.toFixed(2)}% / ` +
          `위치 ${currentDayPosition.toFixed(1)}% / 위치하락 ${dayPositionDrop.toFixed(1)}%p`
        : "")
  };
}
...
let observationPeakPrice = 0;
let observationLowAfterPeak = 0;
let observationPeakDrawdownRate = 0;
let observationRecoveryRate = 0;

if (
  observationPrices.length >= 2 &&
  ...
  for (
    let i = 0;
    i < observationPrices.length;
    i++
  ) {
    if (
      observationPrices[i] >=
      observationPeakPrice
    ) {
      observationPeakPrice =
        observationPrices[i];
  ...
  const afterPeakPrices =
    observationPrices.slice(
      peakIndex
    );

  observationLowAfterPeak =
    afterPeakPrices.length
      ? Math.min(...afterPeakPrices)
      : Number(current.price || 0);

  observationPeakDrawdownRate =
    observationPeakPrice > 0
      ? (
          (
  ...
  observationRecoveryRate =
    observationLowAfterPeak > 0
      ? (
          (
            Number(current.price) -
            observationLowAfterPeak
  ...
  // 고점 대비 -2.5% 이상 무너진 경우
  // 짧은 반등 여부와 관계없이 매수 금지
  if (
    observationPeakDrawdownRate <=
    Number(
      settings
        .openObservationPeakHardBlockRate ??
      -2.50
    )
  ) {
    return {
      ...
      reason:
        `관찰고점 붕괴 ` +
        `${observationPeakDrawdownRate.toFixed(2)}% / ` +
        `고점 ${observationPeakPrice.toLocaleString()}원 / ` +
        `차단기준 ` +
        `${Number(
          settings
            .openObservationPeakHardBlockRate ??
  ...
  // 고점 대비 -1.5 ~ -2.5%
  // 충분한 재상승이 확인된 경우에만 허용
  if (
    observationPeakDrawdownRate <=
    Number(
      settings
        .openObservationPeakCautionDrawdownRate ??
      -1.50
    )
  ) {
    const recoveryOk =
      ...
      Number(
        settings
          .openObservationPeakRecoveryMinRate ??
        0.80
      );
    ...
    const recentRiseOk =
      ...
      Number(
        settings
          .openObservationPeakRecentRiseMinRate ??
        0.10
      );
    ...
    if (
      ...
      return {
        ...
        reason:
          ...
          `${observationPeakDrawdownRate.toFixed(2)}% / ` +
          `저점회복 ` +
          `+${observationRecoveryRate.toFixed(2)}%` +
          `/${Number(
            settings
              .openObservationPeakRecoveryMinRate ??
            0.80
...
if (!momentum.pass) {
  ...
  observationPeakPrice:
    Number(observationPeakPrice || 0),

  observationLowAfterPeak:
    Number(observationLowAfterPeak || 0),

  observationPeakDrawdownRate:
    Number(
      observationPeakDrawdownRate || 0
    ),

  observationRecoveryRate:
    Number(observationRecoveryRate || 0),
...
function judgeOpenBuy(state, item, price, options = {}) {
  ...
  if (
    !ignoreMarketBlocks &&
    marketData.available &&
    marketScore < Number(settings.openMarketAbsoluteBlockScore || 20)
  ) {
    return {
      pass: false,
      _precomputedStrengthen: strengthen,
      reason:
        `OPEN 시장절대차단 / ` +
        `시장 ${marketScore}점 / ` +
        `절대기준 ${Number(settings.openMarketAbsoluteBlockScore || 20)}점`
    };
  }
...
async function paperOpenBuy(state, item, price, reason, judged = {}) {
  ...
  try {
    ...
    const actualBuyAmount = qty * Number(price || 0);
    const preBuyRisk = checkOpenPreBuyRiskBudget(
      state,
      actualBuyAmount
    );

    if (preBuyRisk.blocked) {
      const failReason = preBuyRisk.reason;
      console.log(
        `[OPEN 손실예산 차단] ${name}(${code}) / ${failReason}`
      );
      return {
        ok: false,
        code: "DAILY_LOSS_RISK_BUDGET",
        reason: failReason,
        dailyRisk: preBuyRisk
      };
    }
