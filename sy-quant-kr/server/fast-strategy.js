...
const STRATEGY_VERSION = "1.3.8-MASTER";
...
const SETTINGS = {
  ...
  // 신규매수 가능 시 READY 후보를 먼저 재검증하고, 남는 슬롯으로 WATCH 후보를 보강한다.
  candidateReadyRealtimeCheckCount: 3,
  candidateWatchRealtimeCheckCount: 2,
  ...
  // 1~5번째 진입도 시장이 매우 약할 때는 저점수 FAST만 선별적으로 보류한다.
  // 시장 전체 차단이 아니라 FAST 75점 미만만 거르는 안전장치.
  weakMarketEntryGuardScore: 40,
  weakMarketMinFastScore: 75,
  ...
  // V1.3.6: 발견점수만 1점 부족한 초강력 조기돌파 종목 구제
  earlyBreakoutStrongOverrideEnabled: true,
  earlyBreakoutStrongOverrideMaxFirstChangeRate: 4.0,
  earlyBreakoutStrongOverrideMaxChangeRate: 7.0,
  earlyBreakoutStrongOverrideMinDiscoverScore: 4,
  earlyBreakoutStrongOverrideMinSourceCount: 3,
  earlyBreakoutStrongOverrideMinHotScore: 70,
  earlyBreakoutStrongOverrideMinMomentumScore: 70,
  earlyBreakoutStrongOverrideMinDayPositionRate: 80,
  earlyBreakoutStrongOverrideMinVolumeRatio: 130,
  earlyBreakoutStrongOverrideMinSampleCount: 3,
  earlyBreakoutStrongOverrideMinPricePersistence: 0.75,
  earlyBreakoutStrongOverrideMinVolumePersistence: 0.67,
  ...
  // FAST 단계형 수익잠금:
  // +3% 이상 기록 후 현재수익 +1.0% 이하 -> 이익보호
  // +4% 이상 기록 후 현재수익 +1.5% 이하 -> 더 강한 이익보호
  profitLock3StartProfitRate: 3.0,
  profitLock3FloorProfitRate: 1.0,
  profitLock4StartProfitRate: 4.0,
  profitLock4FloorProfitRate: 1.5,
...
function evaluateCandidate(candidate, snapshot, marketData = {}) {
  ...
  const earlyBreakoutStrongDiscoverOverride = Boolean(
    SETTINGS.earlyBreakoutStrongOverrideEnabled &&
    earlyBreakoutActive &&
    firstChangeRate <= SETTINGS.earlyBreakoutStrongOverrideMaxFirstChangeRate &&
    changeRate <= SETTINGS.earlyBreakoutStrongOverrideMaxChangeRate &&
    discoverScore >= SETTINGS.earlyBreakoutStrongOverrideMinDiscoverScore &&
    sourceCount >= SETTINGS.earlyBreakoutStrongOverrideMinSourceCount &&
    hotScore >= SETTINGS.earlyBreakoutStrongOverrideMinHotScore &&
    momentumScore >= SETTINGS.earlyBreakoutStrongOverrideMinMomentumScore &&
    dayPosition >= SETTINGS.earlyBreakoutStrongOverrideMinDayPositionRate &&
    volumeRatio >= SETTINGS.earlyBreakoutStrongOverrideMinVolumeRatio &&
    persistence.sampleCount >= SETTINGS.earlyBreakoutStrongOverrideMinSampleCount &&
    persistence.pricePersistence >= SETTINGS.earlyBreakoutStrongOverrideMinPricePersistence &&
    persistence.volumePersistence >= SETTINGS.earlyBreakoutStrongOverrideMinVolumePersistence
  );
  ...
  if (earlyBreakoutActive) {
    trackResults.unshift({
      ...
      checks: [
        ...
        [
          "discover",
          discoverScore >= SETTINGS.earlyBreakoutMinDiscoverScore || earlyBreakoutStrongDiscoverOverride,
          earlyBreakoutStrongDiscoverOverride
            ? `초기돌파 발견 ${discoverScore.toFixed(1)}점 / 강한특례`
            : `초기돌파 발견 ${discoverScore.toFixed(1)}점`
        ],
...
function getSellDecision(holding, price, hhmm = getCurrentHHMM()) {
  ...
  // 최고 +4% 이상을 찍은 종목은 +1.5%까지 반납하면 수익을 잠근다.
  if (
    maxProfitRate >= SETTINGS.profitLock4StartProfitRate &&
    maxProfitRate < SETTINGS.trailingStartProfitRate &&
    profitRate <= SETTINGS.profitLock4FloorProfitRate
  ) {
    return { sell: true, type: "FAST_PROFIT_LOCK_SELL", reason: `4% 수익잠금 / 최고 ${maxProfitRate.toFixed(2)}% / 현재 ${profitRate.toFixed(2)}%`, profitRate, maxProfitRate, drawdownRate };
  }

  // 최고 +3% 이상을 찍은 종목은 +1.0%까지 반납하면 수익을 잠근다.
  if (
    maxProfitRate >= SETTINGS.profitLock3StartProfitRate &&
    maxProfitRate < SETTINGS.profitLock4StartProfitRate &&
    profitRate <= SETTINGS.profitLock3FloorProfitRate
  ) {
    return { sell: true, type: "FAST_PROFIT_LOCK_SELL", reason: `3% 수익잠금 / 최고 ${maxProfitRate.toFixed(2)}% / 현재 ${profitRate.toFixed(2)}%`, profitRate, maxProfitRate, drawdownRate };
  }
  if (
...
async function tryFastBuys(state, evaluations, marketData) {
  ...
  const recheckablePool = evaluations
  ...
  if (initialBuyCapacity) {
    // READY는 이미 현재 평가를 통과한 실제 매수 후보이므로 WATCH 시세보강보다 우선한다.
    const readyPool = recheckablePool
      .filter(row => row.evaluation?.pass === true)
      .sort((a, b) => {
        const scoreDiff = toNumber(b.evaluation.fastScore) - toNumber(a.evaluation.fastScore);
        if (scoreDiff) return scoreDiff;
        return toNumber(a.candidate.lastRealtimeCheckAtMs) - toNumber(b.candidate.lastRealtimeCheckAtMs);
      });

    // WATCH는 기존 목적대로 시세 누락/오래된 자료를 우선 보강한다.
    const watchPool = recheckablePool
      .filter(row => row.evaluation?.pass !== true)
      .sort((a, b) => {
    ...
    const readyTargets = readyPool.slice(0, SETTINGS.candidateReadyRealtimeCheckCount);
    const remainingSlots = Math.max(
      0,
      SETTINGS.candidateRealtimeCheckCount - readyTargets.length
    );
    const watchTargetCount = Math.min(
      SETTINGS.candidateWatchRealtimeCheckCount,
      remainingSlots
    );
    const watchTargets = watchPool.slice(0, watchTargetCount);

    // READY가 3개보다 적으면 남은 재조회 슬롯을 WATCH 보강에 추가 사용한다.
    if (readyTargets.length + watchTargets.length < SETTINGS.candidateRealtimeCheckCount) {
      const alreadySelected = new Set(watchTargets.map(row => row.candidate.code));
      const extraWatchTargets = watchPool
        .filter(row => !alreadySelected.has(row.candidate.code))
        .slice(0, SETTINGS.candidateRealtimeCheckCount - readyTargets.length - watchTargets.length);
      watchTargets.push(...extraWatchTargets);
    }

    targets = [...readyTargets, ...watchTargets]
      .slice(0, SETTINGS.candidateRealtimeCheckCount);

    // READY인데 이번 5개 재검증 대상에서 빠진 후보를 분석용으로 남긴다.
    const selectedCodes = new Set(targets.map(row => row.candidate.code));
    const waitingReady = readyPool.filter(row => !selectedCodes.has(row.candidate.code));
    const nowMs = Date.now();
    if (
      waitingReady.length > 0 &&
      nowMs - toNumber(daily.lastReadyWaitLogAtMs) >= 30 * 1000
    ) {
      daily.lastReadyWaitLogAtMs = nowMs;
      const preview = waitingReady
        .slice(0, 3)
        .map(row => `${row.candidate.name}(${row.candidate.code}) FAST ${toNumber(row.evaluation.fastScore).toFixed(1)}`)
        .join(' / ');
      console.log(
        `[FAST READY 대기] ${waitingReady.length}개 / 이번 재검증 ${targets.length}개 / ${preview}`
      );
    }
  } else {
    // 매수슬롯이 닫힌 때는 기존처럼 누락/노후 시세자료만 소량 보강한다.
    targets = recheckablePool
      .filter(row => needsDataOnlyRealtimeRefresh(row.snapshot))
      .sort((a, b) => {
        const missingDiff = Number(hasMissingRealtimeQuoteFields(b.snapshot)) - Number(hasMissingRealtimeQuoteFields(a.snapshot));
        if (missingDiff) return missingDiff;
        const ageDiff = toNumber(a.snapshot?._fastRealtimeEnrichedAtMs) - toNumber(b.snapshot?._fastRealtimeEnrichedAtMs);
      ...
      .slice(0, SETTINGS.candidateDataOnlyRealtimeCheckCount);
  }
  ...
  for (const target of targets) {
    try {
      ...
      const weakMarketEntryBlocked = (
        realtimeEvaluation.pass &&
        buyCapacity &&
        marketData?.available === true &&
        toNumber(marketData.score) < SETTINGS.weakMarketEntryGuardScore &&
        toNumber(realtimeEvaluation.fastScore) < SETTINGS.weakMarketMinFastScore
      );

      if (weakMarketEntryBlocked) {
        target.candidate.status = "WATCH";
        target.candidate.reason =
          `매우약한장 저점수 진입보류 / 시장 ${toNumber(marketData.score).toFixed(1)}점 / ` +
          `FAST ${toNumber(realtimeEvaluation.fastScore).toFixed(1)}/${SETTINGS.weakMarketMinFastScore}`;
        target.candidate.diagnosticReason = target.candidate.reason;
        console.log(
          `[FAST 약세장 진입보류] ${target.candidate.name}(${target.candidate.code}) / ` +
          target.candidate.reason
        );
        continue;
      }
