...
// 2026-09-02 VOLUME 약시장 진입 강화
volumeWeakMarketRequireRebound: true,
volumeWeakMarketScoreThreshold: 40,
volumeWeakMarketRequireDayPositionRecovery: true,
volumeWeakMarketMinDayPositionRecovery: 0.0,
volumeWeakMarketRequireWatchScoreHold: true,
volumeWeakMarketMinWatchScoreDiff: 0.0,
...
function loadState() {
  ...
  // 실제 1차 익절 거래로그를 기준으로 완료 플래그를 복구한다.
  // 서버 주문 처리와 상태병합 순서가 엇갈려도 반복 부분익절을 막는다.
  restoreFirstTakeProfitDoneFromTradeLogs(state);
...
const watchScore =
  ...
  if (existing) {
    ...
    existing.previousWatchScore = Number(
      existing.watchScore || watchScore || 0
    );
 ...
 * 1차 익절 완료상태 복구
 *
 * 주문 API가 paper-state-core.json의 보유수량을 먼저 갱신하거나
 * 동시 상태병합 과정에서 firstTakeProfitDone 필드가 사라져도,
 * 동일 포지션의 실제 FIRST_TAKE_PROFIT 거래로그가 존재하면
 * 다시 1차 익절을 실행하지 않는다.
 */
function restoreFirstTakeProfitDoneFromTradeLogs(state) {
  const logs = Array.isArray(state.tradeLogs)
    ? state.tradeLogs
    : [];

  for (const holding of state.holdings || []) {
    if (holding.firstTakeProfitDone === true) {
      continue;
    }
    ...
    const code = normalizeTradeCode(holding.code);
    ...
    const matched = logs.some(log => {
      ...
      if (
        log.date !== todayKey() ||
        logStrategyGroup !== strategyGroup ||
        normalizeTradeCode(log.code) !== code ||
        logType !== `${strategyGroup}_FIRST_TAKE_PROFIT`
      ) {
        return false;
      }

      // positionId가 양쪽 모두 있으면 반드시 같은 포지션만 인정한다.
      if (
        positionId &&
        log.positionId &&
        String(log.positionId) !== positionId
      ) {
        return false;
      }

      // 구형 로그처럼 positionId가 없을 때는 매수시각 이후 로그만 인정한다.
      const buyTime = Number(
        holding.buyTime ||
        holding.buyTimeMs ||
        0
      );
      const logTime = getTradeLogTimestampMs(log);

      return !buyTime || !logTime || logTime >= buyTime;
    });

    if (matched) {
      holding.firstTakeProfitDone = true;
      holding.firstTakeProfitRecoveredFromLog = true;
    }
  }
}

function getTodayLossExitCount(state, targetStrategyGroup = null) {
  const logs = state.tradeLogs || [];
  const countedBuyPositions = new Set();
  const normalizedTargetStrategyGroup = String(
    targetStrategyGroup || ""
  ).toUpperCase();
  ...
  logs.forEach((log, sellIndex) => {
    ...
    if (
      ...
      !["CORE", "VOLUME"].includes(strategyGroup) ||
      (
        normalizedTargetStrategyGroup &&
        strategyGroup !== normalizedTargetStrategyGroup
      ) ||
      String(log.type || "").endsWith("_BUY") ||
      Number(log.profitRate || 0) >= 0
    ) {
...
function checkStrategyDailyBuyLimit(state, strategyGroup) {
  ...
  // CORE와 VOLUME의 일일 손실매도 횟수는 전략별로 독립 계산한다.
  // CORE의 손실 4건 때문에 실제 거래가 없던 VOLUME까지 신규매수가
  // 중단되는 교차차단을 방지한다.
  const lossCount = getTodayLossExitCount(
    state,
    strategyGroup
  );
  ...
  if (lossLimit > 0 && lossCount >= lossLimit) {
    return {
      blocked: true,
      reason:
        `${strategyGroup} 손실매도 ${lossCount}/${lossLimit}건 도달 / ` +
        `해당 전략 신규매수 중단`
    };
  }
  ...
  return {
    blocked: false,
    reason:
      `${strategyGroup} 하루 매수 ${count}/${limit}건 / ` +
      `${strategyGroup} 손실매도 ${lossCount}/${lossLimit}건`
  };
}
...
function isVolumeCandidateGettingStronger(
  ...
  const recentDayPositionChange =
    Number(current.dayPosition || 0) -
    Number(previous.dayPosition || current.dayPosition || 0);
  ...
  if (history.phase === "PULLBACK") {
    ...
    if (
      reboundRate >= Number(settings.volumeReboundMinRate || 0.30) &&
      recentPriceChangeRate > 0 &&
      (
        settings.volumeWeakMarketRequireDayPositionRecovery !== true ||
        recentDayPositionChange >= Number(
          settings.volumeWeakMarketMinDayPositionRecovery || 0
        )
      )
    ) {
      ...
      history.volumeRetentionRate = volumeRetentionRate;
      history.dayPositionDiff = dayPositionDiff;
      history.recentDayPositionChange = recentDayPositionChange;
...
async function paperBuy(
  ...
  try {
    ...
    if (
      strategyGroup === "VOLUME" &&
      settings.volumeWeakMarketRequireRebound === true &&
      Number(marketScore || 0) <
        Number(settings.volumeWeakMarketScoreThreshold || 40)
    ) {
      const weakVolumeHistory =
        state.volumeCandidateHistory?.[String(item.code || "")] || null;

      if (weakVolumeHistory?.phase !== "REBOUND") {
        console.log(
          `[VOLUME 매수제외] ${name} / 약시장 EARLY_MOMENTUM 직접매수 금지 / ` +
          `시장 ${Number(marketScore || 0).toFixed(1)}점 / ` +
          `현재단계 ${weakVolumeHistory?.phase || "NONE"} / REBOUND 대기`
        );
        return false;
      }

      const recentDayPositionChange = Number(
        weakVolumeHistory?.recentDayPositionChange || 0
      );

      if (
        settings.volumeWeakMarketRequireDayPositionRecovery === true &&
        recentDayPositionChange <
          Number(settings.volumeWeakMarketMinDayPositionRecovery || 0)
      ) {
        console.log(
          `[VOLUME 매수제외] ${name} / 약시장 당일위치 회복 미확인 / ` +
          `직전대비 ${recentDayPositionChange.toFixed(1)}%p`
        );
        return false;
      }

      const previousWatchScore = Number(
        watchItem?.previousWatchScore ??
        candidateWatchScore
      );
      const watchScoreDiff = candidateWatchScore - previousWatchScore;

      if (
        settings.volumeWeakMarketRequireWatchScoreHold === true &&
        watchScoreDiff <
          Number(settings.volumeWeakMarketMinWatchScoreDiff || 0)
      ) {
        console.log(
          `[VOLUME 매수제외] ${name} / 약시장 강화점수 약화 / ` +
          `${previousWatchScore.toFixed(1)}→${candidateWatchScore.toFixed(1)} ` +
          `(${watchScoreDiff.toFixed(1)}) / 재상승 재확인`
        );
        return false;
      }

      console.log(
        `[VOLUME 약시장 최종통과] ${name} / ` +
        `시장 ${Number(marketScore || 0).toFixed(1)}점 / ` +
        `REBOUND +${Number(weakVolumeHistory?.reboundRate || 0).toFixed(2)}% / ` +
        `위치회복 ${recentDayPositionChange >= 0 ? "+" : ""}` +
        `${recentDayPositionChange.toFixed(1)}%p / ` +
        `강화점수 ${previousWatchScore.toFixed(1)}→${candidateWatchScore.toFixed(1)}`
      );
    }
...
async function paperSell(
  ...
  try {
    ...
    // 다음 loadState()에서는 거래로그 기반 복구도 함께 동작한다.
    if (String(sellType || "").includes("_FIRST_TAKE_PROFIT")) {
      holding.firstTakeProfitDone = true;
      holding.firstTakeProfitDoneAtMs = sellRequestedAtMs;
      holding.firstTakeProfitDoneAt = nowText();
    }
...
if (
  ...
  // 2. 1차 익절
  // loadState()에서 거래로그 기반으로 완료 플래그를 복구하므로
  // 동일 포지션은 한 번만 부분익절한다.
  if (
    holding.firstTakeProfitDone !== true &&
