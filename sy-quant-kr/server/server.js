...
function normalizePriceQueueCode(code) {
  return String(code || "")
    .replace(/^A/i, "")
    .replace(/_AL$/i, "")
    .padStart(6, "0");
...
function boostQueuedKiwoomPriceRequest(
  code,
  priority = 0,
  maxQueueWaitMs = 0,
  source = "default"
) {
  const normalizedCode = normalizePriceQueueCode(code);
  const nextPriority = Number(priority || 0);
  const nextMaxWaitMs = Number(maxQueueWaitMs || 0);

  for (const request of kiwoomPriceRequestQueue) {
    if (request.code !== normalizedCode) continue;

    const beforePriority = Number(request.priority || 0);

    if (nextPriority > beforePriority) {
      request.priority = nextPriority;

      if (nextMaxWaitMs > Number(request.maxQueueWaitMs || 0)) {
        request.maxQueueWaitMs = nextMaxWaitMs;
      }

      console.log(
        `[PRICE QUEUE 승격] ${normalizedCode} / ` +
        `${beforePriority}→${nextPriority} / ` +
        `${request.source || "unknown"}→${source}`
      );
    }
...
function runSharedTradingPriceRequest(
  code,
  taskFactory,
  shareGroup = "all-price",
  priority = 0,
  maxQueueWaitMs = 0,
  source = "default"
) {
  const normalizedCode = normalizePriceQueueCode(code);
  const key = `${String(shareGroup || "all-price")}_${normalizedCode}`;

  const existing = tradingPriceInflightByCode.get(key);

  if (existing) {
    boostQueuedKiwoomPriceRequest(
      normalizedCode,
      priority,
      maxQueueWaitMs,
      source
...
if (cached && Date.now() - cached.cachedAt <= freshCacheMaxAgeMs) {
  ...
  return res.json({
    ...
    // SELL 계열은 위험관리상 독립 최신시세를 유지한다.
    // 나머지 현재가 조회는 동일 종목이면 하나의 ka10001 요청을 공유한다.
    const sharedRequestGroup = isSellPriceRequest
      ? null
      : "all-price";
    ...
    const data = isSharedTradingRequest
      ? await runSharedTradingPriceRequest(
          code,
          requestCurrentPrice,
          sharedRequestGroup,
          requestPriority,
